...
public class JDBCAirlineDao implements AirlineDao {
    ...
    @Override
    public ArrayList<Aircraft> showSortedFuelConsumption(int min, int max) {
        ArrayList<Aircraft> resultList = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement
                ("SELECT * FROM CargoAircraft WHERE fuelConsumption BETWEEN ? AND ? ")) {
            ps.setInt(1, min);
            ps.setInt(2, max);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
            ...
            PreparedStatement pss = connection.prepareStatement("SELECT * FROM PassengerAircraft WHERE fuelConsumption BETWEEN ? AND ? ");
            pss.setInt(1, min);
            pss.setInt(2, max);
